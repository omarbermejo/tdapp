...
function TabSlot({
  ...
  const t = useTheme();
  const press = usePressScale({ to: 0.86, haptic: Haptics.ImpactFeedbackStyle.Light });
...
function FloatingTabs({ state, navigation, insets }: BottomTabBarProps) {
  ...
  useEffect(() => {
    if (index < 0) return;
    // .set() y no .value =: el compilador de React trata el shared value como inmutable y
    // asignarle rompe el lint (es el error que arrastra use-press-scale).
            ...
            return (
              <TabSlot
                ...
                onPress={() => {
                  ...
                  if (tab.name !== current && !event.defaultPrevented) navigation.navigate(route.name);
                }}
              />
            );
          })}
