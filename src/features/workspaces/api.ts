...
export const workspacesApi = {
  ...
  members: (token: string, id: number) =>
    request<{ members: Member[] }>(`/workspaces/${id}/members`, {
      headers: bearer(token),
    }),

  /** Con quien ya has trabajado, para no teclear un correo cada vez. */
  collaborators: (token: string) =>
    request<{ collaborators: Collaborator[] }>("/workspaces/collaborators", {
      headers: bearer(token),
    }),
};
...
export const requestsApi = {
  list: (token: string) =>
    request<{ requests: JoinRequest[] }>("/workspaces/requests", {
      headers: bearer(token),
    }),
  ...
  decide: (
    ...
    andInvalidate(
      request<{ approved: boolean }>(
        `/workspaces/${workspaceId}/requests/${personId}`,
        {
          method: "POST",
          headers: bearer(token),
...
export const invitesApi = {
  ...
  create: (
    token: string,
    workspaceId: number,
    to?: { email?: string; personId?: number },
  ) =>
    request<{ invite: Invite; resent: boolean }>(
  ...
  list: (token: string, workspaceId: number) =>
    request<{ invites: Invite[] }>(`/workspaces/${workspaceId}/invites`, {
      headers: bearer(token),
    }),
